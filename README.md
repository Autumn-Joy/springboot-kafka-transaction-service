...
- Refactored `TransactionKafkaListener` to delegate to `TransactionService`

- `TransactionService` now handles the transaction validation and processing 
  - `TransactionService` calls `TransactionIncentiveClient` to post the transaction to the API
    - `TransactionIncentiveClient` uses `RestTemplate` to make the API call
      - `RestClientConfig` creates the `RestTemplate` instance
